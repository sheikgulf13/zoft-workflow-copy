...
export default function AcceptInvitationPage() {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const message = extractAxiosMessage(error) || "Failed to accept invitation";
        ...
        <div className="flex flex-1 flex-col justify-center px-6 py-8 lg:px-12 xl:px-16 max-h-screen overflow-y-auto">
          <div className="mx-auto w-full max-w-md">
            <div className="text-center mb-6 lg:hidden">
              ...
              <h1 className="text-2xl font-bold text-theme-primary">Accept Invitation</h1>
            </div>
            ...
            <div className="bg-theme-form backdrop-blur-md rounded-3xl p-6 shadow-lg">
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" aria-live="polite">
                <div>
                  <label className="mb-3 block text-sm font-semibold text-theme-primary">First Name</label>
                  <div className="relative group">
                    <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                      <User size={18} className="text-theme-tertiary group-focus-within:text-theme-primary transition-colors" />
                    </div>
                    <input
                      type="text"
                  ...
                  {errors.firstName && (
                    <p className="mt-2 text-xs font-medium text-[#ef4a45]">{errors.firstName.message}</p>
                  )}
                </div>

                <div>
                  <label className="mb-3 block text-sm font-semibold text-theme-primary">Last Name</label>
                  <div className="relative group">
                    <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                      <User size={18} className="text-theme-tertiary group-focus-within:text-theme-primary transition-colors" />
                    </div>
                  ...
                  {errors.lastName && (
                    <p className="mt-2 text-xs font-medium text-[#ef4a45]">{errors.lastName.message}</p>
                  )}
                </div>

                <div>
                  <label className="mb-3 block text-sm font-semibold text-theme-primary">Password</label>
                  <div className="relative group">
                    <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                ...
                {!token && (
                  <p className="text-xs text-[#ef4a45] mt-2">Token is missing from the link.</p>
                )}
              </form>
              ...
              <div className="mt-6 text-center">
                <p className="text-theme-secondary text-sm">
                  ...
                  <Link to="/auth/sign-in" className="font-semibold text-[#b3a1ff] hover:text-[#a08fff] transition-colors">
                    Sign in
                  </Link>
                </p>
              </div>
