...
export default function SendVerificationPage() {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (values) => {
    try {
      const baseUrl = getBackendBaseUrl();
      const url = `${baseUrl ? baseUrl.replace(/\/$/, "") : ""}/api/email/send-verification`;
      
      // Make unauthenticated request (no bearer token)
      await http.post(url, {
        email: values.email,
      });
          ...
          <div className="max-w-md">
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-theme-secondary">Secure email verification</span>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-2 h-2 bg-[#8dff8d] rounded-full"></div>
                <span className="text-theme-secondary">Instant delivery</span>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-2 h-2 bg-[#b3a1ff] rounded-full"></div>
                <span className="text-theme-secondary">One-click verification</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-1 flex-col justify-center px-6 py-8 lg:px-12 xl:px-16 max-h-screen overflow-y-auto">
          <div className="mx-auto w-full max-w-md">
            <div className="text-center mb-6 lg:hidden">
              ...
              <h1 className="text-2xl font-bold text-theme-primary">Verify Email</h1>
            </div>

            <div className="hidden lg:block mb-6">
              <h2 className="text-3xl font-bold text-theme-primary mb-2">Send Verification</h2>
              <p className="text-theme-secondary">Enter your email to receive a verification link</p>
            </div>
